...
class GitRemoteStrategy implements RemoteStrategy {
  ...
  private async configure(repository: FolderProxy): Promise<void> {
    ...
    let hasOrigin = true;
    try {
      await this.runner.run("git", ["remote", "get-url", "origin"], { cwd: repository.path });
    } catch {
      hasOrigin = false;
    }
    if (hasOrigin) {
      await this.runner.run("git", ["remote", "set-url", "origin", this.pullUrl], { cwd: repository.path });
    } else {
      await this.runner.run("git", ["remote", "add", "origin", this.pullUrl], { cwd: repository.path });
    }
    await this.runner.run("git", ["remote", "set-url", "--push", "origin", this.pushUrl.trim() || this.pullUrl], { cwd: repository.path });
  }

  async push(repository: FolderProxy): Promise<void> {
    await this.configure(repository);
    await this.runner.run("git", ["push", "-u", "origin", "HEAD"], { cwd: repository.path });
  }

  async pull(repository: FolderProxy): Promise<void> {
    await this.configure(repository);
    const branch = (await this.runner.run("git", ["branch", "--show-current"], { cwd: repository.path })).stdout.trim();
    if (!branch) throw new Error("The backup repository has no current branch to pull.");
    await this.runner.run("git", ["pull", "--ff-only", "origin", branch], { cwd: repository.path });
  }
}
